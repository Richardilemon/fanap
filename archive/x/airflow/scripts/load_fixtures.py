    ...
    try:
        ...
        for row_num, row in enumerate(reader, start=1):
            try:
                ...
                required_fields = ["code", "team_h", "team_a", "kickoff_time"]
        ...
        for fixture_list in fixtures:
            ...
            for match in fixture_list:
                try:
                    ...
                    logger.error(f"Integrity error for fixture {match[0]}: {e}")
                    total_failed += 1
                    connection.rollback()
                    continue
                except psycopg2.DatabaseError as e:
                    logger.error(f"Database error for fixture {match[0]}: {e}")
                    total_failed += 1
                    connection.rollback()
                    continue
