        ...
        for player in players:
            if player["type"] == "dir":  # Filter for directories only
                ...
                try:
                    ...
                    # Transform the 'value' column by dividing by 10
                    if 'value' in df.columns:
                        df['value'] = df['value'] / 10  # Converts 75 → 7.5, 50 → 5.0, etc.
...
dag = DAG(
    ...
    description='A DAG to update FPL data every gameweek',
    schedule_interval='0 0 * * 5',  # Runs every Friday at midnight
)

update_task = PythonOperator(
    task_id='update_fpl',
    python_callable=update_fpl_database,
    dag=dag,
)
